...
class QuestionExtractor:
    ...
    def _clean_question(self, text: str) -> str:
        """
        Clean a single question block - IMPROVED VERSION
        
        Fixes:
        - Better removal of ". ." and ". . ." patterns
        - Better removal of "..." patterns
        ...
        # Find first "A." and cut everything after
        match = re.search(r'\b[A-E]\.\s+', text)
        if match:
            text = text[:match.start()]
        
        # === IMPROVED: Remove ALL trailing dots and spaces ===
        # This fixes ". ." and ". . ." patterns
        text = re.sub(r'[\.\s]+$', '', text)
        
        # Remove ellipsis patterns
        text = re.sub(r'\s*…\s*$', '', text)
        
        # Final cleanup: remove any remaining trailing dots
