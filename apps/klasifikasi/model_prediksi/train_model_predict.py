...
def upload_csv_file():
    """Locate a CSV file in the current working directory (for local runs).

    Behavior:
    - If exactly one CSV exists in cwd, use it.
    - If multiple CSVs exist, try common names (e.g. 'australia dataset.csv'), otherwise ask user to choose.
    - Returns an absolute path to the selected CSV file.
    """
    cwd = os.getcwd()
    csv_files = [f for f in os.listdir(cwd) if f.lower().endswith('.csv')]

    if not csv_files:
        raise FileNotFoundError(f"No CSV files found in current directory: {cwd}")

    if len(csv_files) == 1:
        chosen = csv_files[0]
        print(f"Using CSV found in current directory: {chosen}")
        return os.path.join(cwd, chosen)

    # try some preferred filenames
    preferred = ['australia dataset.csv', 'dataset.csv', 'data.csv']
    for p in preferred:
        if p in csv_files:
            print(f"Using preferred CSV: {p}")
            return os.path.join(cwd, p)

    print("Multiple CSV files found in current directory:")
    for i, fn in enumerate(csv_files, start=1):
        print(f"  {i}. {fn}")

    choice = input("Enter number of file to use (or full filename): ").strip()
    if choice.isdigit():
        idx = int(choice) - 1
        if 0 <= idx < len(csv_files):
            return os.path.join(cwd, csv_files[idx])

    if choice in csv_files:
        return os.path.join(cwd, choice)

    raise ValueError("Invalid selection for CSV file.")
...
def read_data(csv_path: str) -> pd.DataFrame:
    ...
    if detected_text_col:
        ...
        print(f"❌ Could not find text column. Available columns: {list(df.columns)}")
    ...
    if detected_label_cols:
        ...
        print(f"✓ Auto-detected label columns: {LABEL_COLUMNS}")
    else:
        print(f"❌ Could not detect binary label columns. Available columns: {list(df.columns)}")
...
def main():
    ...
    try:
        ...
        try:
            ...
            print(f"✓ Results archived to: {zip_name}")
            print("You can now copy or move this .zip file from the script's working directory.")
        except Exception as e:
            print("Could not create archive of the results:", e)
            print(f"Please manually collect files from the '{OUTPUT_DIR}' folder.")
